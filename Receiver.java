...

{
    ...
    protected void Input(Packet packet) {
        ...
        if (!isCorrupted(packet) && !isOutOfOrder(packet)) {
            ...
            Packet ack = new Packet(seqNum, ackNum,
                                generateChecksum(seqNum, ackNum, 0));
            udtSend(ack);
            expectedSeqNum++;
        ...
        // packet corrupted or out of order, send ACK for highest rcvd seqNum
            ...
            int seqNum = expectedSeqNum - 1;
            int ackNum = seqNum;
            Packet ackForLastSeq = new Packet(seqNum, ackNum,
                                generateChecksum(seqNum, ackNum, 0));
            udtSend(ackForLastSeq);
            expectedSeqNum++;
            System.out.println("RECEIVER SENT: " + ackForLastSeq);
        }
    }
